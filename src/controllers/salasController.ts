...
const adicionar = async (req: Request, res: Response): Promise<void> => {
  if (req.method === 'GET') {
    ...
    try {
      ...
      await SalaService.criar(nome, bloco, andar, parsedNumero, parsedCapacidade);
...
const excluir = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const sala = await SalaService.listarUm(salaId);
    if (!sala) {
...
const editar = async (req: Request, res: Response): Promise<void> => {
  if (req.method === 'GET') {
    ...
    try {
      const { id } = req.params;
      const salaId = parseInt(id, 10);
      await SalaService.editar(salaId, req.body);
      res.redirect('/salas/gerenciar');
    } catch (error: unknown) {
      if (error instanceof Error) {
        res.status(500).send({ message: error.message });
