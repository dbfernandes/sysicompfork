...
// 3. Gerar o PDF
export async function criarAfastamentoPDF(req: Request, res: Response, next: NextFunction) {
    try {
        const id = req.params.id

        const afastamentoPath = path.join(process.cwd(),
        ...
        res.setHeader('Content-Disposition', `inline; filename=${dados!.usuarioNome}.pdf`)
