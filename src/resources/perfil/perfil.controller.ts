...
const editar = async (req: Request, res: Response) => {
  ...
  switch (req.method) {
    ...
    case 'POST': {
      const administrador =
        req.body.administrador && req.body.administrador === 'on' ? 1 : 0;
      const coordenador =
        req.body.coordenador && req.body.coordenador === 'on' ? 1 : 0;
      const secretaria =
        req.body.secretaria && req.body.secretaria === 'on' ? 1 : 0;
      const professor =
        req.body.professor && req.body.professor === 'on' ? 1 : 0;
      const dados = {
        id: id,
