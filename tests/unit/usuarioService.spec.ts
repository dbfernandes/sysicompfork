...
describe('UsuarioService', () => {
  ...
  describe('listarUmUsuario', () => {
    ...
    it('deve lançar um erro se o usuário não for encontrado', async () => {
      ...
      await expect(UsuarioService.listarUmUsuario(999)).rejects.toThrow(
  ...
  describe('adicionar', () => {
    it('deve lançar um erro se houver um erro ao adicionar o usuário', async () => {
      ...
      prismaMock.usuario.create.mockRejectedValue(
        new Error('Erro ao adicionar usuário'),
      );

      await expect(UsuarioService.adicionar(mockUsuario)).rejects.toThrow(
  ...
  describe('alterar', () => {
    ...
    it('deve lançar um erro se houver um erro ao alterar o usuário', async () => {
      ...
      // Mock findFirst e update
      prismaMock.usuario.findFirst.mockResolvedValue(null);
      prismaMock.usuario.update.mockRejectedValue(
        new Error('Erro ao alterar usuário'),
      );

      await expect(UsuarioService.alterar(1, mockUsuario)).rejects.toThrow(
  ...
  describe('RecuperarSenha', () => {
    it('deve lançar um erro se a atualização falhar', async () => {
      ...
      prismaMock.usuario.update.mockRejectedValue(
        new Error('Erro ao recuperar senha'),
      );

      await expect(
        UsuarioService.recuperarSenha('token', mockDate, 1),
      ).rejects.toThrow('Erro ao recuperar senha');
    });
  });
});
