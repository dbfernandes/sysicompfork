/*
  ...
  - You are about to drop the column `tokenResetarSenha` on the `Candidato` table. All the data in the column will be lost.
  - You are about to drop the column `validarTokenResetada` on the `Candidato` table. All the data in the column will be lost.
  ...
  - You are about to drop the column `idCandidate` on the `CandidatoPublicacoes` table. All the data in the column will be lost.
  - Added the required column `idCandidato` to the `CandidatoPublicacoes` table without a default value. This is not possible if the table is not empty.
...
ALTER TABLE `Candidato` DROP PRIMARY KEY,
    DROP COLUMN `cartaDoOrientador`,
    DROP COLUMN `curriculum`,
    DROP COLUMN `propostaDeTrabalho`,
    DROP COLUMN `tokenResetarSenha`,
    DROP COLUMN `validarTokenResetada`,
...
ALTER TABLE `CandidatoPublicacoes` DROP PRIMARY KEY,
    DROP COLUMN `idCandidate`,
    ADD COLUMN `idCandidato` INTEGER NOT NULL,
    ADD PRIMARY KEY (`id`, `idCandidato`);
