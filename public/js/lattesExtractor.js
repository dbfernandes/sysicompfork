...
function get_publicDict(dados) {
  ...
  const localIdx =
    get_index(dados[dtIndx], '_NOME-DO-EVENTO') != ''
      ? get_index(dados[dtIndx], '_NOME-DO-EVENTO')
      : get_index(dados[dtIndx], '_TITULO-');
  ...
  if (integrantesArr.length > 0) {
    for (var i in integrantesArr) {
      ...
      if (
        ...
        papel =
          integrantesArr[i]['_FLAG-RESPONSAVEL'] == 'SIM'
            ? 'Coordenador'
            : 'Participante';
      }
    }
  ...
  if (financiadoresArr) {
    if (financiadoresArr['FINANCIADOR-DO-PROJETO'].length > 0) {
      for (var j in financiadoresArr['FINANCIADOR-DO-PROJETO']) {
        var nomeFinanciador =
          financiadoresArr['FINANCIADOR-DO-PROJETO'][j]['_NOME-INSTITUICAO'];
        financiadores.push(
      ...
      var nomeFinanciador =
        financiadoresArr['FINANCIADOR-DO-PROJETO']['_NOME-INSTITUICAO'];
...
function getCompleteFormData(data, publicCallback) {
  ...
  reader.addEventListener(
    ...
    () => {
      ...
      const instituicoes =
        xmlText['CURRICULO-VITAE']['DADOS-GERAIS']['ATUACOES-PROFISSIONAIS'][
      ...
      const orientacoesConcluida = xmlText['CURRICULO-VITAE']['OUTRA-PRODUCAO']
        ? xmlText['CURRICULO-VITAE']['OUTRA-PRODUCAO']['ORIENTACOES-CONCLUIDAS']
        : null;
